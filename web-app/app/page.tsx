...
function HomeContent() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] bg-background overflow-hidden relative">
      ...
      <div className="w-full max-w-[1600px] mx-auto px-5 sm:px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
        <div className="space-y-8 sm:space-y-10 flex flex-col items-start text-left">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 sm:gap-2.5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-primary/20 bg-primary/5 backdrop-blur-md animate-fade-in-down" style={{ animationDelay: '100ms' }}>
            ...
            <span className="text-xs sm:text-sm font-medium tracking-wide text-primary/80">Internal Tool v1.0</span>
          </div>

          {/* Headline */}
          <div className="space-y-4 sm:space-y-6 max-w-2xl">
            <h1 className="font-sans text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-medium tracking-tighter leading-[0.95] text-foreground animate-fade-in-up" style={{ animationDelay: '200ms' }}>
              Meeting <br/>
              <span className="text-primary italic font-serif">Intelligence</span>
            </h1>
            <p className="text-base sm:text-lg md:text-2xl text-muted-foreground font-light leading-relaxed animate-fade-in-up max-w-lg" style={{ animationDelay: '300ms' }}>
              Automate your notes. Analyze candidate fit. Sync with Lever. <span className="text-foreground font-normal">Seamlessly.</span>
            </p>
          </div>

          {/* CTA */}
          <div className="flex flex-col sm:flex-row gap-4 pt-2 animate-fade-in-up w-full sm:w-auto" style={{ animationDelay: '400ms' }}>
            <Button
              size="lg"
              className="h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base rounded-full gap-3 shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30 transition-all duration-500 ease-smooth hover:scale-[1.02] active:scale-[0.98] bg-primary text-primary-foreground border border-white/10 w-full sm:w-auto"
        ...
        {/* Visual / Orb — hidden on mobile and tablet */}
